...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'L8FQZgFTYNcLd1w',
      'isDeleted':false,
      'username':'Isabel87',
      'email':'Immanuel.Cormier@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.updateOne(User, { 'username':'Isabel87' }, userToBeInserted,  { upsert: true });
    userToBeInserted = {
      'password':'nwmOPHLwy1QnQhQ',
      'isDeleted':false,
      'username':'Darrion_Williamson',
      'email':'Schuyler.Johnston@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.updateOne(User, { 'username':'Darrion_Williamson' }, userToBeInserted,  { upsert: true });
